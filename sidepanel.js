        ...
        if (recognition) {
            try {
                ...
                if (error.name === 'NotAllowedError' || error.name === 'PermissionDeniedError') {
                    errorMessage += 'Please allow microphone access by following these steps:\n\n' +
                        '1. Click the lock/padlock icon in the address bar\n' +
                        '2. Find the microphone setting\n' +
                        '3. Change it from "Block" to "Allow"\n\n' +
                        'Or go to:\n' +
                        'Chrome Settings > Privacy and Security > Site Settings > Microphone\n' +
                        'Find this extension and change the permission to "Allow"';
                } else if (error.name === 'NotFoundError' || error.name === 'DevicesNotFoundError') {
                    errorMessage += 'No microphone device was found. Please ensure a microphone is connected and working.';
                } else if (error.name === 'NotReadableError' || error.name === 'TrackStartError') {
                    errorMessage += 'Your microphone is busy or not working properly. Please check if another application is using it.';
                } else if (error.name === 'OverconstrainedError') {
                    errorMessage += 'Your microphone does not meet the required specifications. Please try a different microphone.';
                } else if (error.name === 'TypeError') {
                    errorMessage += 'Your browser does not support microphone access. Please try a different browser.';
                } else {
                    errorMessage += 'An unexpected error occurred. Please try again or check your microphone settings.';
                }
